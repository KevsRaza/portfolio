<template>
    <header class="flex justify-between items-center p-6 bg-opacity-50 relative z-20">
        <div class="text-3xl font-bold bg-clip-text bg-linear-to-r from-blue-400 to-cyan-400 text-transparent">
            Kevin <span class="font-black">RAZA</span>
        </div>
            ...
            <ul class="flex flex-col items-center space-y-5 md:flex-row md:space-x-5 md:space-y-0">
                <li v-for="item in Menu" :key="item.name" class="group relative">
                    <a :href="item.href"
                        ...
                        {{ item.name }}
                        <span
                            class="absolute bottom-0 left-0 w-0 h-0.5 bg-blue-400 transitioin-all duration-300 group-hover:w-full"></span>
                    </a>
                </li>
            </ul>
            <div class="mt-8 md:mt-0 md:ml-8">
                <a href="https://wa.me/261321537742?text=Hello%20I%20would%20like%20to%20discuss%20a%20project"
                    target="_blank" rel="noopener noreferrer" class="inline-flex items-center gap-2 px-6 py-2.5
